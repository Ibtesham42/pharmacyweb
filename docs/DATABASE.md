...
- **PREMIUM membership approval** (migration `12_membership_approval`): enum `MembershipStatus` (PENDING/APPROVED/REJECTED/SUSPENDED/EXPIRED). `Membership` holds the entitlement record (`status`, **nullable** `expiresAt` set only on approval, `purchaseId`, `reviewedAt`/`reviewedById`); active = `status=APPROVED AND expiresAt>now` (the single gate behind `hasEntitlement`). `MembershipPurchase.membershipStatus` is the per-payment verification state shown in the admin queue. Payment records PENDING; an admin approves to activate. See `docs/AUTH.md`.
