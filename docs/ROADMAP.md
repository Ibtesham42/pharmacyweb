...
| 14 Marketplace | Digital Resources Marketplace (`/store`): free/paid downloads, secure delivery, passwordless buyer accounts, Razorpay + UPI, moderated reviews, admin suite + analytics (`docs/MARKETPLACE.md`). **Phase 2 done:** admin **AI authoring tools** (excerpt/SEO/tags/MCQs/flashcards, incl. PDF grounding), analytics bar charts, bookmarks UX. **Phase 3:** **Thesis & Research Library** (`/library`, ScholarlyArticle) + **Exam-Prep Bundles** (`/exam-prep`, migration `7_add_bundles`) shipped; memberships/PREMIUM designed. | ◐ |
