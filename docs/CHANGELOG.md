...
- **PREMIUM Memberships (Marketplace Phase 3):** an all-access `/membership` — buy a fixed-term **PREMIUM pass** (1/3/12-month plans, set by admin) and download **every** paid and premium resource for the duration; no auto-renewal (re-buy to extend; renewing while active stacks on the remaining time). One-time payment via the same Razorpay-instant + UPI-manual flow. An active membership unlocks all downloads through the existing entitlement check, and **PREMIUM-tier resources are now members-only** (the storefront shows a "Get PREMIUM" CTA instead of "coming soon"). My Account shows membership status + expiry; printable + emailed receipts. **Admin** (Resources → **Memberships**): manage plans (price/duration/badge/active), see active-member count, and verify UPI membership purchases. Added to nav + sitemap (migration `8_add_memberships`). **This completes the marketplace roadmap (Phases 1–3).**
