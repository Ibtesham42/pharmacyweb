...
## 2026-06-18 — Payment idempotency hardening + auth-gated purchases
Stability/correctness pass (no new features). Two issues fixed; rest of the auth/membership/download flow audited and confirmed already-correct (evidence below).

**1. Duplicate receipt email / notification / grant (fixed across all 4 flows).** Root cause: `markPaid`/`adminSet*Status` returned `void`, so every caller (Razorpay `/verify` **and** the `order.paid`/`payment.captured` webhook, plus admin manual-verify) fired the receipt email + notification unconditionally — a Razorpay payment fires both → two emails/notifications. The webhook's `purchase.status !== PAID` pre-read was also non-atomic (a verify+webhook race could double-send). The membership **admin** path was worse: `adminSetMembershipPurchaseStatus` used `update` + granted whenever `status===PAID`, so a repeat click **re-extended** the membership.
  - Fix: `markPaid`/`markPaidByOrderId` now return `boolean` (`updateMany … where status≠PAID` → `count>0`) = *true only on the first transition*; `adminSet{Purchase,BundlePurchase,MembershipPurchase}Status` are now transition-aware (updateMany guard, grant only on real →PAID, return the boolean). All callers gate side-effects on it: `if (await markPaid(...)) void send…ReceiptEmail(...)`. Membership grant stays inside `markPaid`, gated on the same atomic `count>0` → exactly one grant. Donations have no email/notification/grant (already dup-free) — `markPaid` made to return the boolean for a uniform contract. Files: `services/{resource,bundle,membership}-purchases.ts`, `services/donations.ts`; verify routes ×3, `razorpay/{resource,bundle,membership}-webhook` ×3 (removed the now-unused `OrderStatus` import), admin `actions.ts` ×3.
  - **Verified** with a temporary live-DB integration test (then removed): `markPaid` → `true` then `false,false` on retries; membership `expiresAt` identical after the 2nd/3rd call (granted once); admin re-click → `false`, no re-extend; resource purchase transitions once. All test rows cleaned up (verified 0 leftover; memberships back to baseline = 1).

**2. Purchases were not auth-gated server-side (fixed).** Root cause: `/api/{membership/[planId],resources/[slug],bundles/[slug]}/purchase` trusted a free-text `email` from the body via `ensureBuyer` — no session required. The store/exam-prep UIs gated guests (`DownloadGate`), but the **membership page rendered the buy form to guests**, and any of the three APIs could be called directly by a guest (or tied to someone else's email).
  - Fix: each purchase route now calls `getCurrentBuyer()` → `401 "Please sign in to continue."` for guests, and uses the **session** buyer's `id`/`email` (name falls back to the typed name) for the purchase + Razorpay echo — so a purchase/membership is always assigned to the authenticated account, never a spoofable body email. The membership page now passes `authed`; guests see a **"Sign in to go PREMIUM"** link (plans stay visible, per spec) instead of the form.
  - **Verified** (dev): guest `POST` to all three purchase endpoints → `401`; guest browsing of `/membership /store /library /exam-prep /jobs /articles /news /ai` → `200`; membership page shows "Sign in to go PREMIUM"; guest download (and bogus-token download) → `307 /login` (unchanged).

**Audit — already-implemented behaviours (no change made; evidence):** guest browsing of all content (200s); download requires auth for FREE+PAID (`download/route.ts` redirects guests); direct file URLs can't bypass (Cloudinary **authenticated** assets via ~90s signed URLs only); PREMIUM⇒active membership, PAID⇒ownership, all server-side (`hasEntitlement` + `hasActiveMembership`); entitlement keyed by `buyerId`/email (bridged to the account) so it follows the account across devices; re-download from `/account` (`listBuyerPurchases`); header shows account menu + bell when signed-in and hides "Sign in" (`site-header.tsx`).

**Checks:** `typecheck` + `lint` clean; `npm run test` 17/17; `npm run build` exit 0 (all purchase/webhook routes compiled); live-DB idempotency test 3/3.

### Follow-ups
- Membership/MembershipPurchase already store user (via bridged buyer)/plan/payment id/transaction ref/dates/status; a direct `Membership.userId` is possible later but not needed (the email bridge is authoritative).
- The shared purchase form still shows an email field; it's now ignored server-side (session email wins). Could be made read-only when signed in (cosmetic).

## 2026-06-18 — PREMIUM memberships: manual admin-approval workflow
Replaced auto-activation with an admin **approval gate**. Confirmed two product decisions: **all** payments (Razorpay + manual UPI) land **Pending Verification**; renewals **keep access live** (approving the new payment extends).

- **DB — migration `12_membership_approval`** (additive + backfill, applied to Neon via `migrate deploy`): new enum `MembershipStatus {PENDING APPROVED REJECTED SUSPENDED EXPIRED}`; `Membership` gains `status` (default PENDING) + **nullable** `expiresAt` + `purchaseId`/`reviewedAt`/`reviewedById`; `MembershipPurchase` gains `membershipStatus`. **Backfill** set existing `Membership`→APPROVED and PAID purchases→APPROVED (verified: the 1 live member stayed APPROVED — no access regression).
- **Entitlement is now status-gated:** `hasActiveMembership` requires `status=APPROVED && expiresAt>now`; `hasEntitlement`/store/download/bundles inherit it unchanged. `activeMemberCount(Total)` likewise.
- **Grant flow:** `grantMembershipForPurchase` → **`requestMembershipForPurchase`** (records a PENDING membership on the first PAID transition — **no activation**; a renewal that's already active is left live). `markPaid`/`adminSetMembershipPurchaseStatus` call it (idempotency from the prior commit preserved). New `approveMembershipPurchase` / `rejectMembershipPurchase` / `suspendMembership` / `extendMembership`; comp path (`grantCompMembership`/`revokeMembership`, used by `/admin/users`) now maps to APPROVED / SUSPENDED.
- **Receipts/notifications:** payment email/notification now say **"Payment received — pending verification"**; a **new** `sendMembershipApprovedEmail` ("PREMIUM activated") fires on approval. `/membership/receipt/[id]` reflects Pending vs Active.
- **Admin "Membership Verification" console** (`/admin/resources/memberships/purchases`): columns User Name · Email · **User ID** (bridged `buyer.userId`) · Plan · Amount · **Transaction ID** · Method · Purchase Date · Payment Status · **Verification Status**; row actions **Approve / Reject / Suspend / Extend(days) / Payment details**; status filter + a **pending-count** badge on the memberships landing + this page. New actions in `memberships/actions.ts` (requireAdmin + audit + revalidate).
- **User dashboard** (`/account`): membership card now shows **Current Plan · Status · Purchase Date · Approval Status · Expiry Date** with a status badge ("Pending Admin Verification" / "Premium Active" / "Membership Rejected" / "Suspended" / "Expired"); `MEMBERSHIP_STATUS_LABELS`/`_VARIANT` in `lib/marketplace/config.ts`. Also updated `/admin/users/[id]` PREMIUM row + `isMember` checks.
