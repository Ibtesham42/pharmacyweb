...
---
_Last updated: 2026-06-16 — AI models via `add_ai_module` + `3_ai_multimodal_modes`; `4_add_donations` adds the `Donation` model + enums; `5_add_featured_supporters` adds the `FeatureStatus` enum + featured columns/index and backfills existing opt-ins; `6_add_marketplace` (additive) adds the resources marketplace — `Resource`, `ResourceCategory`, `ResourceTag`, `Buyer`, `BuyerAuthToken`, `ResourcePurchase`, `ResourceReview`, `ResourceBookmark`, `ResourceDownload` + enums; `7_add_bundles` (additive) adds exam-prep bundles — `Bundle`, `BundleItem`, `BundlePurchase`; `8_add_memberships` (additive) adds PREMIUM memberships — `MembershipPlan`, `Membership`, `MembershipPurchase` (both applied to Neon) (see `docs/MARKETPLACE.md`); `9_add_user_auth`/`11_add_saved_and_notifications` (unified auth + saved/notifications); `12_membership_approval` (additive + backfill) adds `MembershipStatus` enum, `Membership.status`/nullable `expiresAt`/`purchaseId`/`reviewedAt`/`reviewedById` and `MembershipPurchase.membershipStatus` for the **admin approval workflow** (backfills existing membership→APPROVED, PAID purchases→APPROVED); pgvector `AiEmbedding` deferred._
