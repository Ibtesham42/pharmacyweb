...
import { setMembershipPurchaseStatusAction } from "@/app/admin/(panel)/resources/memberships/actions";
...
export function MembershipPurchasesTable({ purchases }: { purchases: Row[] }) {
  ...
  async function update(id: string, status: OrderStatus) {
    setBusyId(id);
    const res = await setMembershipPurchaseStatusAction(id, status);
    setBusyId(null);
    if (res.ok) {
