...
import { setPurchaseStatusAction } from "@/app/admin/(panel)/resources/actions";
...
export function ResourcePurchasesTable({ purchases }: { purchases: Row[] }) {
  const router = useRouter();
  const [busyId, setBusyId] = useState<string | null>(null);

  async function update(id: string, status: OrderStatus) {
    setBusyId(id);
    const res = await setPurchaseStatusAction(id, status);
    setBusyId(null);
    if (res.ok) {
        ...
        <tbody>
          {purchases.map((p) => (
            <tr key={p.id} className="border-t align-top">
              ...
              <td className="p-2">{p.resource.title}</td>
              <td className="p-2">
                {p.name || "—"}
                <br />
