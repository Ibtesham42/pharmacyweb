...
---
_Last updated: 2026-06-20 — **Unified Authentication & Resource Access Control** (Auth.js: signup/signin/forgot/reset/profile + magic-link, one platform-wide system, roles, protected downloads, user dashboard, saved jobs/articles, notifications, and admin user management — Phases 1–3 complete + a header notification bell, server-side **auth-gated purchases**, **idempotent payments**, a **PREMIUM admin-approval workflow** (payment → Pending Verification → admin Approve → active; `12_membership_approval`), and a daily **membership-expiry sweep** (APPROVED-but-past-expiry → EXPIRED via Vercel Cron `/api/cron/expire-memberships` + a lazy admin-page check), see `docs/AUTH.md`); on top of the AI module (Groq chat + multimodal + Career Copilot + admin resource authoring tools, `docs/AI.md`), the **Donation/Support platform** (`docs/DONATIONS.md`), and the **Digital Resources Marketplace** incl. Library, Exam-Prep Bundles & PREMIUM Memberships (`docs/MARKETPLACE.md`)._
