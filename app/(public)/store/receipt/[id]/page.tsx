...
import { getPurchaseForReceipt } from "@/services/resource-purchases";
...
export const metadata = buildMetadata({
  ...
  path: "/store/receipt",
  description: "Your PharmaCareers resource purchase receipt.",
  noindex: true,
});

export default async function ResourceReceiptPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const p = await getPurchaseForReceipt(id);
  if (!p) notFound();
      ...
      <div className="rounded-2xl border p-6 sm:p-8">
        <div className="text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {paid
              ? "Your resource is unlocked. Use the button below or your account to download."
              : "Your UPI payment is awaiting admin verification. You'll be able to download once confirmed."}
          </p>
        </div>
        ...
        <div className="mt-6 rounded-xl border bg-muted/30 p-5">
          ...
          <dl className="mt-3 space-y-2 text-sm">
            <Row label="Resource" value={p.resource.title} />
