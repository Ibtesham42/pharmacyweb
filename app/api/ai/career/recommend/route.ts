...
import { aiRecommendSchema } from "@/lib/validation";
import { getAiSettings } from "@/services/ai/settings";
import { checkLimits } from "@/services/ai/chat";
import { recommendLearning } from "@/services/ai/career";
...
export async function POST(req: NextRequest) {
  ...
  const { clientId, goal, resumeText } = parsed.data;
  ...
  try {
    return NextResponse.json(await recommendLearning({ clientId, ip }, goal, resumeText));
  } catch {
